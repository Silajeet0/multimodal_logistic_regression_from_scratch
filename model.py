...
class MultiClassLogisticRegression:
    ...
    def softmax(self, score_mtx):
        ...
        # Calculate the exponentials of the stable scores
        exp_scores = np.exp(stable_z)

        # Sum the exponentials for each row (per-sample)
        ...
        # Divide each row's exponentials by that row's sum
    ...
    def fit(self, X, Y, learning_rate = 0.05, epochs = 500, X_val=None, Y_val=None, patience=20, min_delta=1e-4):
        """
            ...
            Args:
                ...
                min_delta (int, optional): The minimum decrease in validation loss required to count
                                           ...
                                           Defaults to 1e-4.
            Returns:
                self: The trained model instance. Helps in method chaining.
        """
        ...
        if X_val is None or Y_val is None:
            print("Starting training (no validation set provided)...")
            for i in range(epochs):
                ...
                if (i + 1) % 100 == 0: #model's progress after 100 epochs
            ...
            for i in range(epochs):
                ...
                if (i + 1) % 10 == 0: # model's progress after 10 epochs
                    print(f"Epoch {i + 1}/{epochs}, Train Loss: {loss:.4f}, Val Loss: {val_loss:.4f}")

                # Check for improvement
                if (best_val_loss - val_loss) > min_delta: # if there is an improvement greater than the threshold
